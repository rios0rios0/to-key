---
name: code-review
description: "Review pull requests and diffs in to-key — the preserved 2012 iOS keygen practice pack in manual-retain-release Objective-C — against the rios0rios0/guide standards, with extra weight on the plist-driven data source, the 32-bit arithmetic semantics, and the educational framing. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `to-key`

`to-key` is an educational reverse-engineering exercise: a small iOS app that reproduces key-generation algorithms for crack-me software, driven by a plist list of options. It is a **preserved historical archive**, discontinued in 2012, targeting iOS SDK 3 with manual retain/release.
...
## What matters most in `to-key`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **The scope is fixed and educational.** This repository reproduces historical crack-me exercises. A pull request that adds a generator for real, currently-licensed software is out of scope — say so, decline, and do not suggest an alternative.
- **New menu entries go in `OptionsList.plist`**, never hard-coded in a view controller. That is the data-source design and the reason the app is extensible at all.
- **Keygen intermediates use `DWORD` (`unsigned long`)** to match the original 32-bit target semantics. Switching to a native 64-bit type changes the wrap-around and silently produces different keys — **Critical**, because the output still looks plausible.
- **Manual retain/release, everywhere.** Balance every `retain`, use `autorelease` pools where the original does; no ARC and no Swift.
- **UI layout is XIB-based.** Do not introduce programmatic layout or Storyboards, and check that an outlet renamed in a header is renamed in the XIB too.
- **4-space indentation and camelCase method names** — follow the existing style rather than reformatting.
- **Treat this as an archive** — no modernisation, no new dependencies, and there is no lint, test, or CI tooling by design.

### Commands a reviewer should be able to quote

```bash
# open "Project/To Key.xcodeproj" in Xcode, target a simulator or device, Cmd+R
# pre-built .app and .ipa are available under Build/
```

## Tests

There is no automated test suite. Verification is building in a simulator and checking each affected generator against its known-good expected output — a keygen change that is not verified against a known value is unreviewable.
